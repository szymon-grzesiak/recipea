...
export const appwriteConfig = {
  ...
  videoCollectionId: "661bb4b631a822bbf89b",
...
export async function isFavorite({ userId, postId }) {
  try {
    const response = await databases.listDocuments(
      databaseId,
      favoriteCollectionId,
      [Query.equal("users", userId), Query.equal("videos", postId)]
    );
...
export async function getFavorites(userId) {
  try {
    ...
    const videoIds = favoritesResponse.documents.map((doc) => doc.videos); // Zakładając, że "videoId" to nazwa pola z ID wideo

    // Pobieranie szczegółów dla każdego ID wideo
    const videoDetailsPromises = videoIds.map((id) =>
      databases.getDocument(databaseId, videoCollectionId, id.$id)
    );
